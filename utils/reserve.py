...
class reserve:
    ...
    def get_submit(
        self, url, times, token, roomid, seatid, captcha="", action=False, value="", username=""
    ):
        ...
        if result.get("success") and username:
            ...
            if reserve_id:
                ...
                try:
                    ...
                    rm.save_reservation(username, str(day), time_segment, reserve_id)
                except ImportError:
                    # 如果 reservation_manager 模块不存在，跳过保存（向后兼容）
                    logging.warning(f"预约成功但无法保存预约ID (reservation_manager模块未安装): reserve_id={reserve_id}")
            else:
                logging.warning(f"预约成功但未找到预约ID: {result}")
